...
echo "Testing max/min valid arguments..."
    ...
    if [[ $? == 1 ]]
    then
        echo -e "${RED}FAILED \"./proj2 1 1 1 1 1000\" returned with 1 even though arguments were valid!${NC}"
        exit 1
    else
        echo -e "${GREEN}OK \"./proj2 1 1 1 1 1000\" ... running ./kontrola-vystupu.sh${NC}"
        if [[ $1 == "-f" ]]
        then
            proj2out_check "-f"
        else
